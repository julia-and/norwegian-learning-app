...
# Deploy the writing correction proxy to Scaleway Serverless Functions
#
# Prerequisites:
...
#      scw init
#
#   3. Create a functions namespace (one-time):
#      scw function namespace create name=norsk-tracker
#      Note the namespace ID from the output.
#
#   4. Create the function (one-time):
...
#
#   5. Set environment variables below or export them:
...
FUNCTION_ID="${SCW_FUNCTION_ID:?Set SCW_FUNCTION_ID}"

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_DIR="$(dirname "$SCRIPT_DIR")"
FUNCTION_DIR="${PROJECT_DIR}/functions/correct"
ZIP_FILE="/tmp/norsk-tracker-correct.zip"
...
echo ""
echo "Then set NEXT_PUBLIC_CORRECTION_API_URL to the endpoint URL before building the frontend."
